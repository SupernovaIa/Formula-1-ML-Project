...
class Encoding:
    ...
    def __init__(self, df: pd.DataFrame, encoding_methods: dict, target_variable: str):
        ...
        self.encoders = {}  # Dictionary to store fitted encoders
        self.mappings = {}  # Dictionary to store frequency encoding mappings
    ...
    def _one_hot_encoding(self):
        ...
        if cols:
            ...
            self.encoders["onehot"] = one_hot_encoder
    ...
    def _ordinal_encoding(self):
        ...
        if cols:
            ...
            # Store fitted encoder
            self.encoders["ordinal"] = ordinal_encoder
    ...
    def _frequency_encoding(self):
        ...
        if cols:
            ...
            for category in cols:
                ...
                frequency = self.df[category].value_counts(normalize=True)
                # Store mapping
                self.mappings[category] = frequency
                # Map values
                self.df[category] = self.df[category].map(frequency)
    ...
    def execute_all_encodings(self):
        ...
        if "frequency" in self.encoding_methods:
            try:
                self._frequency_encoding()
            except Exception as e:
                print(f"Error during Frequency Encoding: {e}")


    def transform(self, new_df):
            """Applies stored encoders/mappings to new data."""
            new_df = new_df.copy()

            if "onehot" in self.encoding_methods and "onehot" in self.encoders:
                cols = self.encoding_methods.get("onehot", [])

                # Perform one-hot encoding
                encoded_data = self.encoders["onehot"].transform(new_df[cols])
                
                # Create a DataFrame for the encoded columns
                df_encoded = pd.DataFrame(encoded_data.toarray(), columns=self.encoders["onehot"].get_feature_names_out())
                
                # Concatenate the encoded columns to the original DataFrame
                new_df = pd.concat([new_df.reset_index(drop=True), df_encoded.reset_index(drop=True)], axis=1)

                # Drop the original columns that were one-hot encoded
                new_df.drop(columns=cols, inplace=True)
                
            if "target" in self.encoding_methods and "target" in self.encoders:
                cols = self.encoding_methods.get("target", [])
                
                # Perform target encoding for the specified columns
                new_df[cols] = self.encoders["target"].transform(new_df[cols])

            if "ordinal" in self.encoding_methods and "ordinal" in self.encoders:
                cols = self.encoding_methods.get("ordinal", {}).keys()

                # Perform the ordinal encoding
                encoded_data = self.encoders["ordinal"].transform(new_df[list(cols)])
                
                # Create a DataFrame for the encoded columns
                df_encoded = pd.DataFrame(encoded_data, columns=self.encoders["ordinal"].get_feature_names_out())
            ...
            if "frequency" in self.encoding_methods and self.mappings:
                cols = self.encoding_methods.get("frequency", [])
                for col in cols:
                    if col in self.mappings:
                        new_df[col] = new_df[col].map(self.mappings[col]).fillna(0)
...
def scale_df(df, cols, method="robust", include_others=False):
    ...
    -------
        ...
        (sklearn.preprocessing.TransformerMixin): The fitted scaler used for transformation.
    """
...
def preprocess(df, encoding_methods, scaling_method, columns_drop=None, target_variable=None, save_objects=False):
    """
    ...
    ----------
        ...
        save_objects (bool): If True, save scaler.pkl in the model directory. Defaults to False.
